...
:::caution
`shaderCost` is an estimated shader-complexity view, not a browser-exposed shader instruction counter. The demo can show WebGPU timestamp render-pass timing when supported, but that timing is calibration evidence, not native ALU/TEX/occupancy data.
:::
