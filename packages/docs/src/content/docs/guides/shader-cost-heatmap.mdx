...
:::caution
`shaderCost` is an estimate. Browsers do not expose portable shader instruction counts through this API, so the view reports source-labeled shader-unit buckets instead of native ALU/TEX counters. When WebGPU timestamp queries are available, the demo may show render-pass timing as calibration metadata.
:::
