...
Main React component for presenting debug views. Import it from `threejs-debug-compose/react`; the root package export stays React-free for core helpers and types.
...
The root package export is for core helpers, planning utilities, TSL compositor helpers, and public types. The `/react` entrypoint contains React/R3F integration and Leva controls.
...
- `useDebugViewsControls()` from `threejs-debug-compose/react`
